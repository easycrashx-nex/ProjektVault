...
function applyStaticTranslations() {
  ...
  setStaticText("#shopSection .shop-layout > div:nth-child(1) .subheading", getUiText("sections.shopOffers"));
  setStaticText("#shopSection .shop-layout > div:nth-child(2) .subheading", getUiText("sections.futureTitle"));
...
function renderArena() {
  ...
  if (!uiState.match) {
    elements.arenaStatus.innerHTML = validation.valid
      ? `<span class="status-pill ok">${getUiText("common.ready")}</span> <span>${getUiText("arena.readyStart")}</span>`
      : `<span class="status-pill warn">${getUiText("arena.notReady")}</span> <span>${validation.messages.join(" ")}</span>`;
    elements.startMatchButton.disabled = !validation.valid;
    elements.resetMatchButton.disabled = true;
    elements.battleHeader.innerHTML = `<span class="status-pill turn">${getUiText("arena.noMatch")}</span><span>${getUiText("arena.noMatchText")}</span>`;
    elements.enemyHeroPanel.innerHTML = `<p class="eyebrow">${getUiText("arena.opponent")}</p><div class="hero-line"><strong>${APP_CONFIG.heroHealth}</strong><span>${getUiText("common.waiting")}</span></div>`;
    elements.playerHeroPanel.innerHTML = `<p class="eyebrow">${getUiText("arena.you")}</p><div class="hero-line"><strong>${APP_CONFIG.heroHealth}</strong><span>${getUiText("common.waiting")}</span></div>`;
    elements.battleLog.innerHTML = `<div class="log-entry">${getUiText("arena.startMatchHint")}</div>`;
  ...
  elements.arenaStatus.innerHTML = matchFinished
    ? `<span class="status-pill ${match.status === "won" ? "ok" : "warn"}">${match.status === "won" ? getUiText("arena.victory") : getUiText("arena.defeat")}</span> <span>${match.statusMessage}</span>`
    : `<span class="status-pill turn">${match.phase === "player" ? getUiText("arena.yourTurn") : getUiText("arena.enemyTurn")}</span> <span>${match.statusMessage}</span>`;

  elements.battleHeader.innerHTML = `
    <span class="meta-chip">${getUiText("arena.round", { turn: match.turn })}</span>
    <span class="meta-chip">${getUiText("arena.mana", { current: match.player.mana, max: match.player.maxMana })}</span>
    <span class="meta-chip">${getUiText("arena.yourDeck", { count: match.player.deck.length })}</span>
    <span class="meta-chip">${getUiText("arena.enemyDeck", { count: match.enemy.deck.length })}</span>
  `;
...
function renderCardModal() {
  ...
  localizedActionRow.append(
    createActionButton(getUiText("card.sellOne"), () => sellCard(card.id, 1), owned < 1),
    createActionButton(getUiText("card.sellDupes"), () => sellCard(card.id, Math.max(0, owned - 1)), owned <= 1),
    createActionButton(getUiText("card.marketSellOne", { price: marketSaleQuote.net }), () => sellCardOnMarket(card.id, 1), owned < 1),
    createActionButton(getUiText("card.marketBuyOne"), () => buyCardOnMarket(card.id), getSave().gold < marketEntry.buyPrice),
    createActionButton(getUiText("card.sellAll"), () => sellCard(card.id, owned), owned < 1),
    createActionButton(getUiText("card.toDeck"), () => addCardToActiveDeck(card.id), !canAddCardToActiveDeck(card.id)),
  );
...
function generateEnemyDeck() {
  ...
  while (deck.length < APP_CONFIG.deckSize && attempts < 500) {
    attempts += 1;
    const preferredPool = Math.random() < 0.68 && units.length ? units : (supports.length ? supports : fallbackPool);
